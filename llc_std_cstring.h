...
namespace llc
{
	...
	stainli	::llc::error_t	toachar (::llc::asc_t	& output, const char * srcstr)	{ 
		const ::llc::vcc			vsrc	= ::llc::tovcc(srcstr);
		llc_necs(output.reserve(vsrc.size())); 
		return (output = vsrc).size(); 
	}
	stainli	::llc::error_t	append 	(::llc::asc_t	& output, const char * srcstr)	{ return output.append(::llc::tovcc(srcstr)); }

	ndstain	::llc::vcc		tolabel (const char * srcstr)	{ return ::llc::label(::llc::tovcc(srcstr)); }
	ndstain	::llc::asc_t	toachar (const char * srcstr)	{ return ::llc::tovcc(srcstr); }
} // namespace llc
