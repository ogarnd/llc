...
namespace llc
{
	...
	//stin	::llc::error_t	tovcc   (::llc::vcc		& output, const Stream	& srcstr)	{ return (output = ::llc::tovcc(srcstr)).size(); }
	//stin	::llc::error_t	tolabel (::llc::vcc		& output, const Stream	& srcstr)	{ return (output = ::llc::label(::llc::tovcc(srcstr))).size(); }
	//stin	::llc::error_t	toachar (::llc::asc_t	& output, const Stream	& srcstr)	{ llc_necs(output.reserve(srcstr.length())); return (output = ::llc::tovcc(srcstr)).size(); }
	//stin	::llc::error_t	append 	(::llc::asc_t	& output, const Stream	& srcstr)	{ return output.append(::llc::tovcc(srcstr)); }
