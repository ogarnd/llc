...
namespace llc
{
	tplt_T	err_t				saveView			(au8 & output, const view<T> & viewToSerialize)	{
		packedu32						counterValue		= viewToSerialize.size();
	...
	tplt_T				err_t	loadView			(vcu8 & input, apod<T> & output) { 
	...
	tplt<tpnm T> stainli err_t	loadView			(vci8 & input, apod<T> & output) { return loadView(*(vcu8*)& input, output); }
	tplt<tpnm T> stainli err_t	loadView			(vcc  & input, apod<T> & output) { return loadView(*(vcu8*)& input, output); }

	err_t						keyValConstStringSerialize	(const view<const TKeyValConstChar> & keyVals, const vcvcc & keysToSave, au8 & output);
} // namespace
