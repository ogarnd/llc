...
namespace llc
{
	...
	ndstinx char		digit_ascii				(char remainder)								noexcpt	{ return remainder + ((remainder < 10) ? '0' : '7'); }
	ndstinx char		digit_ascii				(uint64_t value, uint8_t base)					noexcpt	{ return digit_ascii(char(value % base)); }
	tplt<uint8_t exp, tpnm TValue>
	ndstinx	char		digit					(const TValue value, const TValue base = 10)	noexcpt	{ return ::llc::digit_ascii(value / ::llc::cpow<exp>(base), (uint8_t)base); }
} // namespace
