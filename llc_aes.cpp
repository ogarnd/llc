...
static	void						KeyExpansion							(uint8_t* RoundKey, const uint8_t * Key, ::llc::AES_LEVEL level)		{
	...
	case ::llc::AES_LEVEL_192: Nk = 6; Nr = 12; break;
	case ::llc::AES_LEVEL_256: Nk = 8; Nr = 14; break;
	}
...
static	void										MixColumns								(state_t* state)												{
	for (uint8_t i = 0; i < 4; ++i) {
		...
		uint8_t												Tm;
		Tm												= (*state)[i][0] ^ (*state)[i][1] ; Tm = ::llc::xtime(Tm);  (*state)[i][0] ^= Tm ^ Tmp ;
		Tm												= (*state)[i][1] ^ (*state)[i][2] ; Tm = ::llc::xtime(Tm);  (*state)[i][1] ^= Tm ^ Tmp ;
		Tm												= (*state)[i][2] ^ (*state)[i][3] ; Tm = ::llc::xtime(Tm);  (*state)[i][2] ^= Tm ^ Tmp ;
		Tm												= (*state)[i][3] ^ t ;              Tm = ::llc::xtime(Tm);  (*state)[i][3] ^= Tm ^ Tmp ;
	}
}
...
static uint8_t									Multiply								(uint8_t x, uint8_t y)											{
	return
		( ((y		& 1) * x)
		^ ((y >> 1	& 1) * ::llc::xtime(x))
		^ ((y >> 2	& 1) * ::llc::xtime(::llc::xtime(x)))
		^ ((y >> 3	& 1) * ::llc::xtime(::llc::xtime(::llc::xtime(x))))
		);
}
...
static	void									Cipher									(state_t* state, uint8_t * RoundKey, ::llc::AES_LEVEL level)		{
	...
	case ::llc::AES_LEVEL_192: Nr = 12; break;
	case ::llc::AES_LEVEL_256: Nr = 14; break;
	}
...
/* Public functions:                                                         */
void											llc::aesECBEncrypt					(::llc::SAESContext * ctx, uint8_t * buf)								{ Cipher	((state_t*)buf, ctx->RoundKey.begin(), ctx->Level); } // Cipher encrypts the PlainText with the Key using AES algorithm.
void											llc::aesECBDecrypt					(::llc::SAESContext * ctx, uint8_t * buf)								{ InvCipher	((state_t*)buf, ctx->RoundKey.begin(), ctx->Level); } // InvCiper decrypts the PlainText with the Key using AES algorithm.
void											llc::aesCBCEncryptBuffer			(::llc::SAESContext * ctx, uint8_t * buf, uint32_t length)	{
	uint8_t												* Iv								= ctx->Iv;
	for (uintptr_t i = 0; i < length; i += ::llc::AES_SIZEBLOCK) {
		XorWithIv(buf, Iv);
		Cipher((state_t*)buf, ctx->RoundKey.begin(), ctx->Level);
		Iv												= buf;
		buf												+= ::llc::AES_SIZEBLOCK;
		//printf("Step %d - %d", i/16, i);
	}
	memcpy(ctx->Iv, Iv, ::llc::AES_SIZEBLOCK);	// store Iv in ctx for next call */
}

void											llc::aesCBCDecryptBuffer			(::llc::SAESContext * ctx, uint8_t * buf, uint32_t length)			{
	uint8_t												storeNextIv	[::llc::AES_SIZEBLOCK];
	for (uintptr_t i = 0; i < length; i += ::llc::AES_SIZEBLOCK) {
		memcpy(storeNextIv, buf, ::llc::AES_SIZEBLOCK);
		InvCipher((state_t*)buf, ctx->RoundKey.begin(), ctx->Level);
		XorWithIv(buf, ctx->Iv);
		memcpy(ctx->Iv, storeNextIv, ::llc::AES_SIZEBLOCK);
		buf												+= ::llc::AES_SIZEBLOCK;
	}
}
...
void											llc::aesCTRXCryptBuffer				(::llc::SAESContext* ctx, uint8_t * buf, uint32_t length)			{
	...
	for (uint32_t i = 0; i < length; ++i, ++bi) {
		if (bi == ::llc::AES_SIZEBLOCK) {// we need to regen xor compliment in buffer */
			memcpy(buffer, ctx->Iv, ::llc::AES_SIZEBLOCK);
			Cipher((state_t*)buffer, ctx->RoundKey.begin(), ctx->Level);
			for(bi = (::llc::AES_SIZEBLOCK - 1); bi >= 0; --bi) {	// Increment Iv and handle overflow
...
::llc::error_t			llc::aesEncode			(vcu0_c & messageToEncrypt, vcu0_c & encryptionKey, ::llc::AES_LEVEL level, ::llc::au0_t & outputEncrypted)	{
	...
	for(uint32_t iVal = 0; iVal < ::llc::AES_SIZEIV; ++iVal)
		iv[iVal]											= (uint8_t)(::llc::noise1DBase(::llc::timeCurrentInUs()) * fraction);

	llc_necs(llc::aesEncode(messageToEncrypt, iv, encryptionKey, level, outputEncrypted));
	llc_necs(outputEncrypted.resize(outputEncrypted.size() + ::llc::AES_SIZEIV));
	memcpy(&outputEncrypted[outputEncrypted.size() - ::llc::AES_SIZEIV], iv, ::llc::AES_SIZEIV);
	return 0;
}
...
::llc::error_t			llc::aesEncode		(vcu0_c & messageToEncrypt, vcu0_c & iv, vcu0_c & encryptionKey, ::llc::AES_LEVEL level, ::llc::au0_t & outputEncrypted) {
	...
	llc_necs(outputEncrypted.resize(messageToEncrypt.size() + (paddingRequired ? paddingRequired : ::llc::AES_SIZEBLOCK), (int8_t)0));
	...
	::llc::SAESContext										aes;
	::llc::aesInitCtxIV(&aes, (const uint8_t*)encryptionKey.begin(), level, iv.begin());
	::llc::aesCBCEncryptBuffer(&aes, (uint8_t*)outputEncrypted.begin(), outputEncrypted.size());
	return 0;
}
...
::llc::error_t			llc::aesDecode		(vcu0_c & messageEncrypted, vcu0_c & iv, vcu0_c & encryptionKey, ::llc::AES_LEVEL level, ::llc::au0_t & outputDecrypted) {
	...
	::llc::SAESContext										aes;
	::llc::aesInitCtxIV(&aes, (const uint8_t*)encryptionKey.begin(), level, iv.begin());
	::llc::aesCBCDecryptBuffer(&aes, (uint8_t*)outputDecrypted.begin(), outputDecrypted.size());
	if(outputDecrypted[outputDecrypted.size() - 1] == 0)
		llc_necs(outputDecrypted.resize(outputDecrypted.size() - ::llc::AES_SIZEBLOCK));
	else {
		const int32_t											padCunt									= outputDecrypted[outputDecrypted.size() - 1];
		const int32_t											sizeOriginal							= outputDecrypted.size() - padCunt;
		llc_necs(outputDecrypted.resize(sizeOriginal));
	}
	return 0;
}
