...
namespace llc
{
    ...
    tplt_TInTOut    err_t   packFilter0 (apod<TOut> & filtered, view<const TIn> unfiltered)                         { return unfiltered.for_each([&filtered](const TIn & element) { return filtered.push_back(element); } ); }
    tplt_TInTOut    err_t   packFilter1 (apod<TOut> & filtered, view<const TIn> unfiltered)                         { return unfiltered.for_each([&filtered](const TIn & element) { return filtered.push_back(element); } ); }
    tplt_TInTOut    err_t   packFilter2 (apod<TOut> & filtered, view<const TIn> unfiltered)                         { return unfiltered.for_each([&filtered](const TIn & element) { return filtered.push_back(element); } ); }
    tplt_TInTOut    err_t   packFilter3 (apod<TOut> & filtered, view<const TIn> unfiltered)                         { return unfiltered.for_each([&filtered](const TIn & element) { return filtered.push_back(element); } ); }
