...
#define COLOR_RGB_OPERATOR_MEMBERS(color_struct_type) \
	...
	struct SColorRGBA : public ::llc::rgba8 {
		...
		cnstxpr	SColorRGBA		operator *		(const SColorRGBA & color)	const	noexcept	{ return {(u8)::llc::clamped(r * (int)color.r, 0, 255)	, (u8)::llc::clamped(g * (int)color.g, 0, 255), (u8)::llc::clamped(b * (int)color.b, 0, 255), a};	}
		cnstxpr	SColorRGBA		operator +		(const SColorRGBA & color)	const	noexcept	{ return {(u8)::llc::clamped(r + (int)color.r, 0, 255)	, (u8)::llc::clamped(g + (int)color.g, 0, 255), (u8)::llc::clamped(b + (int)color.b, 0, 255), a};	}
		cnstxpr	SColorRGBA		operator *		(double scalar)				const	noexcept	{ return {(u8)::llc::clamped(r * scalar, 0.0 , 255.0)		, (u8)::llc::clamped(g * scalar, 0.0,   255.0), (u8)::llc::clamped(b * scalar, 0.0,   255.0), a};	}
		cnstxpr	SColorRGBA		operator /		(double scalar)				const				{ return {(u8)::llc::clamped(r / scalar, 0.0 , 255.0)		, (u8)::llc::clamped(g / scalar, 0.0,   255.0), (u8)::llc::clamped(b / scalar, 0.0,   255.0), a};	}
	};	// struct
	...
	struct SColorBGRA : public ::llc::bgra8 {
		...
		cnstxpr	bool			operator==		(uint32_t other)			const	noexcept	{ return other == *((const uint32_t*)this);			}
		cnstxpr	SColorBGRA		operator*		(const SColorBGRA & color)	const	noexcept	{ return {(u8)::llc::min(b * (int)color.b, 255)	, (u8)::llc::min(g * (int)color.g, 255)	,(u8)::llc::min(r * (int)color.r, 255), a};			}
		cnstxpr	SColorBGRA		operator+		(const SColorBGRA & color)	const	noexcept	{ return {(u8)::llc::min(b + (int)color.b, 255)	, (u8)::llc::min(g + (int)color.g, 255)	,(u8)::llc::min(r + (int)color.r, 255), a};			}
		cnstxpr	SColorBGRA		operator-		(const SColorBGRA & color)	const	noexcept	{ return {(u8)::llc::clamped(b - (int16_t)color.b, 0, 255), (u8)::llc::clamped(g - (int16_t)color.g, 0, 255),(u8)::llc::clamped(r - (int16_t)color.r, 0, 255), a};	}
		cnstxpr	SColorBGRA		operator*		(float scalar)				const	noexcept	{ return {(u8)::llc::clamped(b * scalar, 0.0f, 255.0f)	, (u8)::llc::clamped(g * scalar, 0.0f, 255.0f)	,(u8)::llc::clamped(r * scalar, 0.0f, 255.0f), a};			}
		cnstxpr	SColorBGRA		operator/		(float scalar)				const				{ return {(u8)::llc::clamped(b / scalar, 0.0f, 255.0f)	, (u8)::llc::clamped(g / scalar, 0.0f, 255.0f)	,(u8)::llc::clamped(r / scalar, 0.0f, 255.0f), a};			}
		cnstxpr	SColorBGRA		operator*		(double scalar)				const	noexcept	{ return {(u8)::llc::clamped(b * scalar, 0.0,  255.0 )	, (u8)::llc::clamped(g * scalar, 0.0 , 255.0 )	,(u8)::llc::clamped(r * scalar, 0.0, 255.0),	a};				}
		cnstxpr	SColorBGRA		operator/		(double scalar)				const				{ return {(u8)::llc::clamped(b / scalar, 0.0,  255.0 )	, (u8)::llc::clamped(g / scalar, 0.0 , 255.0 )	,(u8)::llc::clamped(r / scalar, 0.0, 255.0),	a};				}
				SColorBGRA&		operator*=		(float scalar)						noexcept	{ b = (u8)::llc::clamped(b * scalar, 0.0f, 255.0f); g = (u8)::llc::clamped(g * scalar, 0.0f, 255.0f); r = (u8)::llc::clamped(r * scalar, 0.0f, 255.0f);	return *this; }
				SColorBGRA&		operator/=		(float scalar)									{ b = (u8)::llc::clamped(b / scalar, 0.0f, 255.0f); g = (u8)::llc::clamped(g / scalar, 0.0f, 255.0f); r = (u8)::llc::clamped(r / scalar, 0.0f, 255.0f);	return *this; }
				SColorBGRA&		operator*=		(double scalar)						noexcept	{ b = (u8)::llc::clamped(b * scalar, 0.0,  255.0 ); g = (u8)::llc::clamped(g * scalar, 0.0 , 255.0 ); r = (u8)::llc::clamped(r * scalar, 0.0, 255.0);		return *this; }
				SColorBGRA&		operator/=		(double scalar)									{ b = (u8)::llc::clamped(b / scalar, 0.0,  255.0 ); g = (u8)::llc::clamped(g / scalar, 0.0 , 255.0 ); r = (u8)::llc::clamped(r / scalar, 0.0, 255.0);		return *this; }
	...
	struct SColorRGB : public ::llc::rgb8 {
		...
		cnstxpr	bool			operator !=		(const SColor16   & other)	const	noexcept	{ return (SColor16)(*this) != other;					}
		cnstxpr	SColorRGB		operator *		(float scalar)				const	noexcept	{ return {(u8)::llc::clamped(r * scalar, 0.0f, 255.0f), (u8)::llc::clamped(g * scalar, 0.0f, 255.0f),	(u8)::llc::clamped(b * scalar, 0.0f, 255.0f)}; }
		cnstxpr	SColorRGB		operator /		(float scalar)				const				{ return {(u8)::llc::clamped(r / scalar, 0.0f, 255.0f), (u8)::llc::clamped(g / scalar, 0.0f, 255.0f),	(u8)::llc::clamped(b / scalar, 0.0f, 255.0f)}; }
		cnstxpr	SColorRGB		operator *		(double scalar)				const	noexcept	{ return {(u8)::llc::clamped(r * scalar, 0.0,  255.0),  (u8)::llc::clamped(g * scalar, 0.0,  255.0),	(u8)::llc::clamped(b * scalar, 0.0,  255.0)};  }
		cnstxpr	SColorRGB		operator /		(double scalar)				const				{ return {(u8)::llc::clamped(r / scalar, 0.0,  255.0),  (u8)::llc::clamped(g / scalar, 0.0,  255.0),	(u8)::llc::clamped(b / scalar, 0.0,  255.0)};  }
		cnstxpr	SColorRGB		operator *		(const SColorRGB  & color)	const	noexcept	{ return {(u8)::llc::clamped(r * (int)color.r, 0, 255), (u8)::llc::clamped(g * (int)color.g, 0, 255), (u8)::llc::clamped(b * (int)color.b, 0, 255)};			}
		cnstxpr	SColorRGB		operator +		(const SColorRGB  & color)	const	noexcept	{ return {(u8)::llc::clamped(r + (int)color.r, 0, 255), (u8)::llc::clamped(g + (int)color.g, 0, 255), (u8)::llc::clamped(b + (int)color.b, 0, 255)};			}
		cnstxpr	SColorRGBA		operator *		(const SColorRGBA & color)	const	noexcept	{ return {(u8)::llc::clamped(r * (int)color.r, 0, 255), (u8)::llc::clamped(g * (int)color.g, 0, 255), (u8)::llc::clamped(b * (int)color.b, 0, 255), color.a};	}
		cnstxpr	SColorRGBA		operator +		(const SColorRGBA & color)	const	noexcept	{ return {(u8)::llc::clamped(r + (int)color.r, 0, 255), (u8)::llc::clamped(g + (int)color.g, 0, 255), (u8)::llc::clamped(b + (int)color.b, 0, 255), color.a};	}
	};	// struct

	// Stores BGR color channels
	struct SColorBGR : public ::llc::color_bgr<u8> {
		...
		cnstxpr	bool			operator ==		(const SColor16   & other)	const	noexcept	{ return (SColor16)(*this) == other;					}
		cnstxpr	bool			operator !=		(const SColor16   & other)	const	noexcept	{ return (SColor16)(*this) != other;					}
		cnstxpr	SColorBGR		operator *		(float scalar)				const	noexcept	{ return {(u8)::llc::clamped(b * scalar, 0.0f, 255.0f), (u8)::llc::clamped(g * scalar, 0.0f, 255.0f), (u8)::llc::clamped(r * scalar, 0.0f, 255.0f)};	}
		cnstxpr	SColorBGR		operator /		(float scalar)				const				{ return {(u8)::llc::clamped(b / scalar, 0.0f, 255.0f), (u8)::llc::clamped(g / scalar, 0.0f, 255.0f), (u8)::llc::clamped(r / scalar, 0.0f, 255.0f)};	}
		cnstxpr	SColorBGR		operator *		(double scalar)				const	noexcept	{ return {(u8)::llc::clamped(b * scalar, 0.0,  255.0) , (u8)::llc::clamped(g * scalar, 0.0,  255.0) , (u8)::llc::clamped(r * scalar, 0.0,  255.0)};	}
		cnstxpr	SColorBGR		operator /		(double scalar)				const				{ return {(u8)::llc::clamped(b / scalar, 0.0,  255.0) , (u8)::llc::clamped(g / scalar, 0.0,  255.0) , (u8)::llc::clamped(r / scalar, 0.0,  255.0)};	}
		cnstxpr	SColorBGR		operator *		(const SColorBGR  & color)	const	noexcept	{ return {(u8)::llc::clamped(b * (int)color.b, 0, 255) , (u8)::llc::clamped(g * (int)color.g, 0, 255)	, (u8)::llc::clamped(r * (int)color.r, 0, 255)};			}
		cnstxpr	SColorBGR		operator +		(const SColorBGR  & color)	const	noexcept	{ return {(u8)::llc::clamped(b + (int)color.b, 0, 255) , (u8)::llc::clamped(g + (int)color.g, 0, 255)	, (u8)::llc::clamped(r + (int)color.r, 0, 255)};			}
		cnstxpr	SColorBGRA		operator *		(const SColorBGRA & color)	const	noexcept	{ return {(u8)::llc::clamped(b * (int)color.b, 0, 255) , (u8)::llc::clamped(g * (int)color.g, 0, 255)	, (u8)::llc::clamped(r * (int)color.r, 0, 255), color.a};	}
		cnstxpr	SColorBGRA		operator +		(const SColorBGRA & color)	const	noexcept	{ return {(u8)::llc::clamped(b + (int)color.b, 0, 255) , (u8)::llc::clamped(g + (int)color.g, 0, 255)	, (u8)::llc::clamped(r + (int)color.r, 0, 255), color.a};	}
