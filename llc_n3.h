...
#pragma pack(push, 1)	// You can read about pragma pack() here: https://www.google.com/search?q=pragma+pack
	...
	struct n3 {
		...
		inlcxpr	Tn3			Clamp				(const Tn3 & min, const Tn3 & max)	const	noexcept	{ return {::llc::clamped(x, min.x, max.x), ::llc::clamped(y, min.y, max.y), ::llc::clamped(z, min.z, max.z)}; }
		...
		Tn3&				RotateX				(double theta)											{
			const ::llc::SSinCos	pairSinCos			= ::llc::getSinCos(theta);
