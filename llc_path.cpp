...
::llc::error_t			llc::pathCreate				(const ::llc::vcc & pathName, const char separator) {
	...
	do {
		...
		ree_if(0 == strncpy_s(folder, pathName.begin(), (offsetBar < 0) ? pathName.size() : offsetBar), "String buffer overflow? Path size: %u.", pathName.size());
		if(0 == strcmp(".", folder))
			continue;
#if defined(LLC_WINDOWS)
...
::llc::error_t			llc::pathList				(const ::llc::vcc & pathToList, ::llc::aachar & output, bool listFolders, const ::llc::vcc extension)	{
	...
	do if(	0 != strcmp(fdFile.cFileName, curDir)
		...
		snprintf(bufferFormat, ::llc::size(bufferFormat) - 2, "%%.%us/%%s", withoutTrailingSlash.size());
		...
		if(0 == extension.size() || (extension.size() < viewPath.size() && 0 == strncmp(viewPath.end() - extension.size(), extension.begin(), ::llc::min(extension.size(), viewPath.size())))) {
			...
			llc_necall(output.push_back(::llc::vcs{sPath, (uint32_t)lenPath}), "%s", "Failed to push path to output list.");
		}
		//output.push_back(0)
	}
...
::llc::error_t			llc::pathList				(const ::llc::vcc & pathToList, ::llc::SPathContents & pathContents, const llc::vcc extension)						{
	...
	snprintf(bufferFormat, ::llc::size(bufferFormat) - 2, "%%.%us/*.*", withoutTrailingSlash.size());
	...
	dir						= opendir(withoutTrailingSlash.begin());
	while ((drnt = readdir(dir)) != NULL) {
		::llc::apod<char>			name						= ::llc::vcs{drnt->d_name, (uint32_t)-1};
		if (name != curDir && name != parDir) {
			int32_t						lenPath						= snprintf(sPath, ::llc::size(sPath) - 2, "%s/%s", withoutTrailingSlash.begin(), drnt->d_name);
