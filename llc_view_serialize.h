...
namespace llc
{
	...
	tplT	stin	err_t	loadPOD			(vcc  & input, T & output)	{ return loadPOD (*(vcu0_t*)& input, output); }
	//
	...
	tplT	stin	err_t	loadUInt		(vcc  & input, T & output)	{ return loadUInt (*(vcu0_t*)& input, output); }
	//
	...
	tplT	stin	err_t	viewRead		(view<const T> & headerToRead, const vcc  & input)	{ return viewRead<const T, const char   >(headerToRead, input); }
	...
	tplT	stin	err_t	loadView	(vcc  & input, view<T> & output) { return loadView(*(vcu0_t*)& input, output); }
	//
	tplt<tpnm T, tpnm TByte>
	...
	tplT	stin	err_t	viewReadLegacy	(view<const T> & headerToRead, const vcc  & input)	{ return viewReadLegacy<const T, const char   >(headerToRead, input); }
