...
namespace llc
{
	tplT			err_t	loadPOD			(vcu8 & input, T & output) { 
		rees_if(input.byte_count() < sizeof(T));
		memcpy(&output, input.begin(), sizeof(T));
		llc_necs(input.slice(input, sizeof(T)));
		return sizeof(T);
	}
	tplT	stainli	err_t	loadPOD			(vci8 & input, T & output)	{ return loadPOD (*(vcu8*)& input, output); }
	tplT	stainli	err_t	loadPOD			(vcc  & input, T & output)	{ return loadPOD (*(vcu8*)& input, output); }
	//
	tplT			err_t	loadUInt		(vcu8 & input, T & output)	{
	...
	tplT	stainli	err_t	loadUInt		(vci8 & input, T & output)	{ return loadUInt (*(vcu8*)& input, output); }
	tplT	stainli	err_t	loadUInt		(vcc  & input, T & output)	{ return loadUInt (*(vcu8*)& input, output); }
	//
	...
	tplT	stainli	err_t	viewRead		(view<const T> & headerToRead, const vcu8 & input)	{ return viewRead<const T, const uint8_t>(headerToRead, input); }
	tplT	stainli	err_t	viewRead		(view<const T> & headerToRead, const vci8 & input)	{ return viewRead<const T, const int8_t >(headerToRead, input); }
	...
	//
	tplT			err_t	loadView		(vcu8 & input, view<const T> & output) { 
		uint32_t					bytesRead		= 0;
		llc_necs(bytesRead = viewRead(output, input)); 
		llc_necs(input.slice(input, bytesRead));
		return 0;
	}
	tplT	stainli	err_t	loadView	(vci8 & input, view<T> & output) { return loadView(*(vcu8*)& input, output); }
	tplT	stainli	err_t	loadView	(vcc  & input, view<T> & output) { return loadView(*(vcu8*)& input, output); }
	//
	tplt<tpnm T, tpnm TByte>
	...
	tplT	stainli	err_t	viewReadLegacy	(view<const T> & headerToRead, const vcu8 & input)	{ return viewReadLegacy<const T, const uint8_t>(headerToRead, input); }
	tplT	stainli	err_t	viewReadLegacy	(view<const T> & headerToRead, const vci8 & input)	{ return viewReadLegacy<const T, const int8_t >(headerToRead, input); }
